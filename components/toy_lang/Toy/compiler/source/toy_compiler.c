...
bool checkNodeInTree(Toy_ASTNode *tree, Toy_ASTNode *node) {
    ...
    switch (tree->type) {
        ...
        case TOY_AST_NODE_TERNARY:
            return checkNodeInTree(tree->ternary.condition, node) || checkNodeInTree(tree->ternary.thenPath, node)
                    || checkNodeInTree(tree->ternary.elsePath, node);
        ...
        case TOY_AST_NODE_FOR:
            return checkNodeInTree(tree->pathFor.preClause, node) || checkNodeInTree(tree->pathFor.condition, node)
                    || checkNodeInTree(tree->pathFor.postClause, node) || checkNodeInTree(tree->pathFor.thenPath, node);
...
static Toy_Opcode Toy_writeCompilerWithJumps(Toy_Compiler *compiler, Toy_ASTNode *node, void *breakAddressesPtr, void *continueAddressesPtr, int jumpOffsets,
        Toy_ASTNode *rootNode) {
    ...
    switch (node->type) {
        ...
        case TOY_AST_NODE_BINARY: {
            ...
            if (override != TOY_OP_EOF && node->binary.opcode >= TOY_OP_VAR_ASSIGN && node->binary.opcode <= TOY_OP_VAR_MODULO_ASSIGN) {
                ...
                if (node->binary.left->type == TOY_AST_NODE_BINARY && node->binary.right->type == TOY_AST_NODE_BINARY
                        && node->binary.left->binary.opcode == TOY_OP_INDEX && node->binary.right->binary.opcode == TOY_OP_INDEX) {
            ...
            if (node->binary.opcode == TOY_OP_INDEX && rootNode->type == TOY_AST_NODE_BINARY
                    && (rootNode->binary.opcode >= TOY_OP_VAR_ASSIGN && rootNode->binary.opcode <= TOY_OP_VAR_MODULO_ASSIGN)
                    && !checkNodeInTree(rootNode->binary.right, node)) {
        ...
        case TOY_AST_NODE_GROUPING: {
            ...
            Toy_Opcode override = Toy_writeCompilerWithJumps(compiler, node->grouping.child, breakAddressesPtr, continueAddressesPtr, jumpOffsets,
                    node->grouping.child);
            if (override != TOY_OP_EOF) { //compensate for indexing & dot notation being screwy
                compiler->bytecode[compiler->count++] = (unsigned char) override; //1 byte
            }
        ...
        case TOY_AST_NODE_BLOCK: {
            ...
            for (int i = 0; i < node->block.count; i++) {
                Toy_Opcode override = Toy_writeCompilerWithJumps(compiler, &(node->block.nodes[i]), breakAddressesPtr, continueAddressesPtr, jumpOffsets,
                        &(node->block.nodes[i]));
                if (override != TOY_OP_EOF) { //compensate for indexing & dot notation being screwy
                    compiler->bytecode[compiler->count++] = (unsigned char) override; //1 byte
                }
        ...
        case TOY_AST_NODE_VAR_DECL: {
            ...
            Toy_Opcode override = Toy_writeCompilerWithJumps(compiler, node->varDecl.expression, breakAddressesPtr, continueAddressesPtr, jumpOffsets,
                    rootNode);
            if (override != TOY_OP_EOF) { //compensate for indexing & dot notation being screwy
                compiler->bytecode[compiler->count++] = (unsigned char) override; //1 byte
            }
        ...
        case TOY_AST_NODE_IF: {
            ...
            if (node->pathIf.elsePath) {
                ...
                Toy_Opcode override = Toy_writeCompilerWithJumps(compiler, node->pathIf.elsePath, breakAddressesPtr, continueAddressesPtr, jumpOffsets,
                        rootNode);
                if (override != TOY_OP_EOF) { //compensate for indexing & dot notation being screwy
                    compiler->bytecode[compiler->count++] = (unsigned char) override; //1 byte
                }
        ...
        case TOY_AST_NODE_FN_RETURN: {
            ...
            for (int i = 0; i < node->returns.returns->fnCollection.count; i++) {
                Toy_Opcode override = Toy_writeCompilerWithJumps(compiler, &node->returns.returns->fnCollection.nodes[i], breakAddressesPtr,
                        continueAddressesPtr, jumpOffsets, rootNode);
                if (override != TOY_OP_EOF) { //compensate for indexing & dot notation being screwy
                    compiler->bytecode[compiler->count++] = (unsigned char) override; //1 byte
                }
